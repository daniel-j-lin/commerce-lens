...
def run_r7_diagnostic(*, request: DiagnosticTestRequest, evidence_inputs: DiagnosticEvidenceInputSet,
                      ...
                      r7_repository: R7Repository, authority_registry: PreTestAuthorityRegistry) -> R7ServiceOutcome:
    ...
    method = R7_METHOD_REGISTRY.authenticate(request.method, handoff.family_id, handoff.family_version, handoff.family_fingerprint)
    if request.normalized_parameters != method.fixed_parameters:
        raise ValueError("R7_GATE_PARAMETER_OVERRIDE")
    required_judgments = set(handoff.requirement_judgment_refs)
    if not required_judgments.issubset({item.requirement_judgment_ref for item in evidence_inputs.bindings}):
        raise ValueError("R7_GATE_BLOCKING_JUDGMENT_UNBOUND")
    ...
    r7_repository.persist(R7ArtifactType.POSTTEST_EVALUATION, evaluation)
    # Mandatory round-trip authentication before returning any analytical outcome.
    r7_repository.load(R7ArtifactType.POSTTEST_EVALUATION, evaluation.evaluation_event_id)
